...
class SendUserReport extends Command
{
     ...
     * @var string
     */
    protected $signature = 'milesreport:send-user-report {user?}';
    ...
    private function generateSheet($userId, $userName, $lastReport) {
        $entries = Entry::whereUserId($userId)->whereDate('created_at', '>', $lastReport);
        $sheet = \Excel::create('Miles-Report-' . str_replace(' ', '-', $userName), function ($excel) use($entries) {
